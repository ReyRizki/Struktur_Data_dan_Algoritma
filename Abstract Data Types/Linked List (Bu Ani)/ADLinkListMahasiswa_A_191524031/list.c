...
boolean isNameExist(List L, char name[])
{
	address P = First(L);
	boolean found = false;
...
void insertMahasiswa(List *L, infotype X)
{
	address P = Alokasi(X);
	address trans = First(*L);
	address prev;
	boolean done = false;
	...
	{
		while ((trans != Nil) && (done == false))
		{
			if ((compareName(Info(P), Info(trans)) < 0) && (NbElmt(*L) == 1))
			{
				InsertFirst(L, P);
				done = true;
			}
			else if (compareName(Info(P), Info(trans)) < 0)
			{
...
void deleteDuplicate(List *L, address pivot)
{
	address prev = pivot;
	address p = Next(prev); 
	address pdel;
	...
	while (p != Nil)
	{
		if (compareName(Info(p), Info(pivot)) == 0)
		{
